...
{
    // ✅ DO NOT start LFCLK manually — BLE SoftDevice already does that safely

    // Configure RTC2 (8 Hz tick)
    CAL_RTC->TASKS_STOP  = 1;
    CAL_RTC->PRESCALER   = 0xFFF;                  // 8 Hz tick (32.768kHz / (4095+1))
    ...
    NVIC_ClearPendingIRQ(CAL_RTC_IRQn);
...
void nrf_cal_set_time(uint32_t year, uint32_t month, uint32_t day,
                      uint32_t hour, uint32_t minute, uint32_t second)
{
    static time_t uncal_difftime, difftime, newtime;
    time_struct.tm_year = year - 1900;
    time_struct.tm_mon  = month - 1;   // months are 0–11
...
{
    time_t return_time = m_time + CAL_RTC->COUNTER / 8;
...
struct tm *nrf_cal_get_time_calibrated(void)
{
    ...
    if (m_calibrate_factor != 0.0f) {
        ...
        calibrated_time = m_last_calibrate_time + 
            (time_t)((float)(uncalibrated_time - m_last_calibrate_time) * 
                     m_calibrate_factor + 0.5f);
...
{
    static char cal_string[32];
    struct tm *t = calibrated ? nrf_cal_get_time_calibrated() : nrf_cal_get_time();
    snprintf(cal_string, sizeof(cal_string),
